...
public interface SubjectOfferingRepository extends JpaRepository<SubjectOffering, Long> {
    ...
    @Query("SELECT DISTINCT so FROM SubjectOffering so LEFT JOIN FETCH so.subject LEFT JOIN FETCH so.teacher LEFT JOIN FETCH so.meetingTimes WHERE so.semester.id = :semesterId")
