...
public interface PlannedRegistrationRepository extends JpaRepository<PlannedRegistration, Long> {
    ...
    @Query("""
            ...
            """)
    Optional<PlannedRegistration> findByStudentIdAndSemesterIdAndSubjectIdWithDetails(Long studentId, Long semesterId, Long subjectId);
    ...
    @Query("""
            DELETE FROM PlannedRegistration pr
            WHERE pr.student.id = :studentId
              AND pr.semester.id = :semesterId
              AND pr.subject.id = :subjectId
            """)
    int deleteByStudentIdAndSemesterIdAndSubjectId(Long studentId, Long semesterId, Long subjectId);
}
