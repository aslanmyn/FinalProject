...
public class PublicProfessorController {
    ...
    public String professors(Model model) {
        model.addAttribute("professors", teacherRepository.findAllByOrderByNameAsc());
        return "professors/list";
    }

    @GetMapping("/professors/{id}")
    public String professorProfile(@PathVariable Long id, Model model) {
        var professor = teacherRepository.findById(id);
        if (professor.isEmpty()) {
            return "redirect:/professors";
        }
        var teacher = professor.get();
        model.addAttribute("teacher", teacher);
        model.addAttribute("currentSections", subjectOfferingRepository.findByTeacherId(teacher.getId()).stream()
